...
if [ $Dedup_System == "cowbtree" ]; then
    ...
    RAID_Size=`sudo blockdev --getsz ${RAID_Device_Name}`
    Dedup_GC_Block_Limit=${RAID_Size}
fi

./create_dmdedup.sh ${Device_Count_in_RAID} ${RAID_Device_Name} ${META_SIZE_PER_DEVICE} \
...
echo -e "sd=sd1,lun=${Dedup_Device_Name},openflags=o_direct,threads=4,size=${device_size}g" \
        "\nwd=wd1,sd=sd1,xfersize=128k,seekpct=0,rdpct=0" \
...
Workload="homes"    # webvm homes mail-3
Thread_Count=2
