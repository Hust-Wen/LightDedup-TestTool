...
void *workload_read(void *arg) {
    ...
    FILE* cur_workload = NULL;
    int cur_workload_id = 0;
    
    if(thread_info->trace_id == 0) {
        cur_workload_id = 1;
        sprintf(trace_name, "%s%d%s", thread_info->trace_path, cur_workload_id, ".blkparse");
        assert_fopen(cur_workload, trace_name, "r", "workload_read thread", NULL);
    }
    else {
    ...
    while(running_time < MAX_RUNNING_TIME) {
        fgets(input_line, 40960, cur_workload);
        int space_n = 0;
        ...
        if(space_n == 8) {
            ...
            else if (io_information.lba + io_information.size < device_size) {
                io_information.lba = (io_information.lba >> 3) << 12;     //lba (sector, 512B) -> offset (B)

                pthread_mutex_lock(&queue_mutex_lock);
                while(enQueue(&IO_queue, &io_information) == FALSE) {
        ...
        if(feof(cur_workload)) {
            ffprintf("workload %d finish! end_fp:%ld\n", cur_workload_id, ftell(cur_workload));
            fclose(cur_workload);
            if(thread_info->trace_id == 0) {
                cur_workload_id++;
                if(cur_workload_id == 22)   break;
                sprintf(trace_name, "%s%d%s", thread_info->trace_path, cur_workload_id, ".blkparse");
                assert_fopen(cur_workload, trace_name, "r", "workload_read thread", NULL);
            }
        }
...
void *print_info_per_second(void *arg) {
    ...
    while(workload_finish < thread_count) {
        sleep(1);
        fprintf(IOPS_fp, "%ld TotalIOPS:%d ReadIOPS:%d WriteIOPS:%d SumReadIOPS:%ld(%.2fGB) SumWriteIOPS:%ld(%.2fGB), total:%.2fGB\n", running_time++, 
